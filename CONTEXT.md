...
| Term | Meaning | Notes |
|---|---|---|
| **会员 Staff** | A shop member — the money-side actor (top-up + checkout). Carries a 手动会员等级 `level` (`normal`=普站 / `gold`=星站, single-source `STAFF_LEVELS`; lists sort 星站-first). Display word 「员工」→「会员」 2026-07-10; 「金站」→「星站」 2026-08-04（标识符 `gold` 不变）. Identifier `Staff`/`staff_id` unchanged. | Soft-deleted via `voided_at`. |
...
- **导航** — three tabs: 记账 (bookkeeping, home) → 汇总 (summary) → 管理 (manage). 记账 is member-centric (search a member → 充值/出库, see their 余额); 汇总 is the supervision view (库存卡 + 综合流水 + 单数零售聚合); 管理 has four segments (会员｜商品｜补货｜配置)；会员/商品段顶栏左「包含删除」、右「导出」xlsx（见 manage-export）。
...
- **管理页导出 manage-export (2026-08-04)** — 吸收未完成的 export-pipeline：公共 `ExportJob`/`runExport`/`useExport`（写盘钉死 `expo-file-system/legacy`）+ `formatCentsAsYuan`（`MoneyText` 共用）；会员/商品各一 xlsx（`xlsx@0.18.5`），列与 PRD 钉死；失败 `toast.error`，取消分享非错。见 [.scratch/2026-08-04-manage-export/](.scratch/2026-08-04-manage-export/)。
- **管理页导入 manage-import (2026-08-04)** — 会员/商品/补货顶栏「导入」→ 根 Stack `import-form`（`kind` + 可选 `confirmExtra`）；xlsx 模板（表头+一行示例）/`expo-document-picker`（`copyToCacheDirectory: false` + 作用域外 uri 先 `copyAsync` 入体验 cache）→ 预览（长字段省略即可）→ bulk mutation（MutationQueue、单次 toast）；按姓名/名称去重只新建。真机收尾见 [#02](.scratch/2026-08-04-manage-import/02-manage-import-smoke.md)。
...
- **会员化改名 + 会员等级 (2026-07-10)** — 全局展示词「员工」→「会员」（代码标识符 `Staff`/`staff_id`/`staff` 表/路由不动，复刻「出库→出单」先例）；`Staff` 增手动 `level` 字段（`normal`=普站 / `gold`=星站，单源 `STAFF_LEVELS`），仓储列表星站优先排序，管理表单提供等级选择器（默认普站、可手动改），列表行/`staff-row`/会员详情展示等级徽标（星站 accent、普站省略）。`level` 走既有审计；schema 加列 + v2 迁移 `ALTER ... DEFAULT 'normal'` 回填老会员为普站（v1 staff DDL 冻结为历史字面量以规避动态 DDL 的 duplicate-column 陷阱，`ColDef.default` 使 `createTableSql` 与 ALTER 的 DEFAULT 对称）。
...
- **管理页导出 PRD**: [.scratch/2026-08-04-manage-export/01-manage-export.md](.scratch/2026-08-04-manage-export/01-manage-export.md)（specs 01–04）
- **管理页导入 PRD**: [.scratch/2026-08-04-manage-import/01-manage-import.md](.scratch/2026-08-04-manage-import/01-manage-import.md)（specs 01–03）；真机收尾 bug [#02](.scratch/2026-08-04-manage-import/02-manage-import-smoke.md)（specs 04–05）
