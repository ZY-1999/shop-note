...
| **单数·零售 Bundles·Retail** | `splitBundleRetail(amountCents, unitPriceCents) → { bundles: floor(a/u), retail: a%u }` — pure single-source helper. Applied to a checkout's `Σ line_amount` against its own `unit_price_snapshot`. | invariant #4 (derived). |
