...
- **适用边界 / 踩坑**：**任何给 5 张既有表（staff/product/stock_record/stock_record_item/audit_log）加列**都适用——这是默认要走的路，不是特例。冻结的是**已发布的历史版本**该表的 `CREATE` 字面量；`COLUMNS`/`SCHEMA` 仍含新列（drift-guard 绑定的是这两者，**不**覆盖 `MIGRATIONS` 字面量，所以冻结合法）。**不要**在已发布迁移里直接留 `createTableSql(table)`——`COLUMNS` 一旦加列它就漂移、会和 `ALTER` 撞列。**若目标表曾在清库重建版（如 v3）里用动态 `createTableSql` 重建，后续再加列时须同时冻结该重建版中的 CREATE**（不只冻 v1）——否则全新库跑到重建版已含新列，后续 `ALTER` 仍会 duplicate-column（例：2026-08 checkout-self-use 给 `stock_record` 加 `self_use`，双冻 v1+v3）。替代方案（给 `runMigrations` 加列存在性 guard、跑到 `ALTER` 前先 `PRAGMA table_info` 判存）更重、要改 `runMigrations` 签名，未采纳。
- **验证**：[src/data/expo-sqlite-migration.test.ts](src/data/expo-sqlite-migration.test.ts) 断言冻结字面量 `!== createTableSql(...)`、对应 `ALTER ... DEFAULT ...` 形状正确；`createTableSql` 快照含新列 DEFAULT。真实 `ALTER` 执行仅靠**设备 smoke**（ADR-0004，Jest 不覆盖），发布前须手跑：老库升级 existing 行得回填默认值、全新库建表+迁移后列存在。
- **关联**：ADR-0003（DDL 与 registry 共用单源）、ADR-0004（真实 SQL → device smoke）、ADR-0008（清库重建后仍可能回冻该版 CREATE）。CONTEXT.md「会员化改名 + 会员等级」与「自用」条目亦有概述。
