 ...
 * - `cents < 0` → `欠货 ¥X.XX` in the danger token (negative = staff owes goods).
 * - `cents > 0` → `¥X.XX` in the success token.
 * - `0` → `¥0.00` in the neutral textSecondary token.
 *
 * Cents are integer 分; yuan is cents/100 to two decimals. The 欠货 form shows the
 * absolute amount (the debt magnitude) — the danger color + 欠货 prefix carry the sign.
 */
export function MoneyText({ cents, ...rest }: { cents: Cents } & TextProps) {
  const theme = useTheme();
  if (cents < 0) {
    return (
      <Text style={[{ color: theme.danger }]} {...rest}>
        欠货 ¥{(-cents / 100).toFixed(2)}
      </Text>
    );
  }
  if (cents > 0) {
