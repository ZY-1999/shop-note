...
- Children:
  ...
  - `summary` (#08, rewritten #05/page-refactor) — a single time-range-scoped view: 时间段 selector (本月/上月/本周/上周) → 库存卡 (as-of-now `useShopAggregate`, range-independent) → 流水 (range-scoped `useDailyFlow` day×staff, staff rows expand to records via `useStockRecords`). Replaced the old four-segment switcher (overview/dailyFlow/byStaff/byProduct). Main module: [summary-tab.tsx](../../../src/components/summary-tab.tsx). Entry: `summary` tab. Feature CodeMap: pending. Status: `confirmed`.
