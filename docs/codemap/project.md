...
- Last updated: `2026-08-04` (manage-export spec #03: `buildStaffWorkbook` + StaffManage 顶栏「导出」; lib `xlsx@0.18.5`)
...
- Children:
  ...
  - `manage` (#09) — staff & product CRUD (search / create / edit / soft-delete / restore) + cost-price revaluation; staff 顶栏「包含删除」+「导出」xlsx (manage-export #01/#03). Main module: [manage-tab.tsx](../../../src/components/manage-tab.tsx). Entry: `manage` tab. Feature CodeMap: pending. Status: `confirmed`.
...
- Children:
  ...
  - `src/export/` — shared export pipeline (manage-export #02/#03). Responsibility: [types.ts](../../../src/export/types.ts) (`ExportJob`), [run-export.ts](../../../src/export/run-export.ts) (availability gate → build → `expo-file-system/legacy` cache write → `expo-sharing`; swallows `User canceled`), [build-staff-workbook.ts](../../../src/export/build-staff-workbook.ts) (`buildStaffWorkbook` → base64 xlsx; `staffExportFilename`). Independent of `src/data/` and UI. Key deps: `expo-file-system/legacy`, `expo-sharing`, `xlsx@0.18.5`.
