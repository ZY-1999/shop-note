...
- Last updated: `2026-08-04` (manage-import #04/#05 smoke: DocumentPicker `copyToCacheDirectory: false` + scoped `copyAsync`; templates header+one example row; restock filterBar 同行)
- Updated: `2026-08-04` (manage-import #03: kind=restock + useImportRestocks + 补货顶栏导入; template 商品名称/数量)
- Updated: `2026-08-04` (manage-import #01: kind-param import shell + staff import; `expo-document-picker`; `src/import/`)
...
- Children:
  ...
  - `manage` (#09) — staff & product CRUD (search / create / edit / soft-delete / restore) + cost-price revaluation; staff/product 顶栏「导入｜导出」、restock 顶栏「导入」xlsx (manage-export #01/#03/#04; manage-import #01 staff / #02 product / #03 restock). Main modules: [manage-tab.tsx](../../../src/components/manage-tab.tsx) + [import-form.tsx](../../../src/components/import-form.tsx). Entries: `manage` tab + root Stack `import-form`. Feature CodeMap: pending. Status: `confirmed`.
...
- Children:
  ...
  - `src/hooks/` — the data-flow layer (ADR-0005). Responsibility: [reads.ts](../../../src/hooks/reads.ts) (one `useQuery` per read), [mutations.ts](../../../src/hooks/mutations.ts) (gate-serialized writes incl. `useImportStaff` / `useImportProducts` / `useImportRestocks`, family-root invalidation), [query-keys.ts](../../../src/hooks/query-keys.ts) (the single registry), [mutation-queue.ts](../../../src/hooks/mutation-queue.ts) (serialization gate), [use-export.ts](../../../src/hooks/use-export.ts) (`useMutation` over `runExport`), `use-theme.ts`. Key deps: `@tanstack/react-query`.
  ...
  - `src/import/` — import pure logic (manage-import #01–#05). Responsibility: staff/product/restock template/parse/preview; each `build*-import-template` emits **header + exactly one example row** (#05); product [build-product-import-template.ts](../../../src/import/build-product-import-template.ts) (`商品导入模板.xlsx` 名称+单价), [parse-product-import-workbook.ts](../../../src/import/parse-product-import-workbook.ts), [preview-product-import.ts](../../../src/import/preview-product-import.ts) (title trim / voided clash / yuan→cents); restock [build-restock-import-template.ts](../../../src/import/build-restock-import-template.ts) (`补货导入模板.xlsx` 商品名称+数量; 示例名含「示例」), [parse-restock-import-workbook.ts](../../../src/import/parse-restock-import-workbook.ts), [preview-restock-import.ts](../../../src/import/preview-restock-import.ts) (active-product match / voided vs missing / qty / in-file dup). Key dep: `xlsx@0.18.5`.
  - `src/lib/` — shared pure helpers (no React). Responsibility: [format-cents-as-yuan.ts](../../../src/lib/format-cents-as-yuan.ts) (`formatCentsAsYuan` — MoneyText + xlsx builds), [parse-yuan-to-cents.ts](../../../src/lib/parse-yuan-to-cents.ts) (`parseYuanToCents` — inverse for product import; manage-import #02).
...
- Entries:
  ...
  - UI / routes:
    ...
    - `src/app/import-form.tsx` → kind-parameterized import shell (manage-import #01/#02/#03; `kind=staff|product|restock`).
...
- Children:
  ...
  - Storage / filesystem: on-device SQLite via `expo-sqlite` — `ExpoSqliteAdapter.open()` (WAL + versioned migrations) is the device store. Tests use `InMemoryAdapter`; the device smoke uses a dedicated `shop_note_smoke.db` so production data is never touched (ADR-0004). Export write path uses `expo-file-system/legacy` (`cacheDirectory` + `writeAsStringAsync`) — not the SDK 57 default entry (deprecated APIs throw at runtime). Import pick uses `expo-document-picker` with **`copyToCacheDirectory: false`**; if the uri is `content://` or outside experience `cacheDirectory`, `copyAsync` into scoped cache then legacy `readAsStringAsync` (manage-import #04 — Expo Go host `DocumentPicker/` cache is not readable by the sandbox).
