...
- Last updated: `2026-08-04` (manage-export spec #02: `src/export/` write+share pipeline + `formatCentsAsYuan`; `MoneyText` uses the pure helper; `useExport` mutation wrapper)
...
- Children:
  ...
  - `src/export/` — shared export pipeline (manage-export #02). Responsibility: [types.ts](../../../src/export/types.ts) (`ExportJob`), [run-export.ts](../../../src/export/run-export.ts) (availability gate → build → `expo-file-system/legacy` cache write → `expo-sharing`; swallows `User canceled`). Independent of `src/data/` and UI. Key deps: `expo-file-system/legacy`, `expo-sharing`.
