...
- Validation Entry:
  ...
  - RNTL harness: [render.tsx](../../../src/testing/render.tsx) mounts components under the real providers backed by a real `InMemoryAdapter` (never mocked Repos — ADR-0006). Platform / outbound IO mocks allowed: `expo-router`, native pickers, SignatureModal canvas, and `expo-file-system/legacy` + `expo-sharing` (or `runExport`) for export UI tests. [async.ts](../../../src/testing/async.ts) provides `waitForSync` / `flushPending` — RNTL v14's `findBy*`/`waitFor` wrap each poll in `act`, which overlaps the next `fireEvent`'s act and leaks timers across tests; these helpers poll without act (see Risk Areas).
