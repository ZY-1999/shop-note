...
- **流水行（`FlowEventRow` checkout）**：自用时展示「自用」标识；bundles/retail 由父组件按规则传入（自用应为 0）。**今日出库流水行本就不展示出库金额**（只渲染时间 +「出库 N 单」+ 零售；`amountCents` 入参未用于展示）——本功能不改变该约定；出库金额继续只在 `FlowSummary`「出库 ¥」与详情等处体现。
