...
- [ ] 补货第一行工具条右侧仅「导入」；进入 `kind=restock` — 入口
- [ ] 模板 `补货导入模板.xlsx` 列：商品名称、数量（无备注列）；经同一 `useExport`/`runExport` 管道 — 模板
- [ ] 经 #01 确认区扩展点注入整批备注（可空）写入本次每一笔补货；确认文案「确认导入 n 个…」含可导入条数 — 备注与确认
- [ ] 商品名称 trim 匹配**有效**商品；不存在/已删除失败；数量非法失败；文件内同名后者失败；一行一笔 `in`/`-1`/单条目 — 校验与语义
- [ ] 确认只写可导入行；`useImportRestocks` 同 #01 bulk 约定；invalidate 与 `useCreateStockRecord` 同家族；成功 toast + 回列表 — 写入
- [ ] 测试用 InMemory **自备有效商品 seed**（不依赖 #02 先导入）— fixture
