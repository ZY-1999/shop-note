...
操作员在汇总页做对账/备份时，时间段只能用「本月/上月/本周/上周」四段芯片，不能按任意起止日选，也不能一键「近 10 天」；页面内容不够一屏时底部会露出灰色区域。导出管道已在管理页落地，但汇总没有导出入口，无法按当前时间段一次导出库存、入库、充值出库等对账用 Excel。

## Solution

1. **白底铺满**：汇总 tab 根列表铺满屏幕白底（`theme.background`），不再露出灰底；只改汇总页。
...
1. 作为操作员，我想汇总页整屏白底，以便内容少时也不看到灰色空白。
...
- 汇总根 `FlatList`：`style`/`contentContainerStyle` 使用 `theme.background`（白），并保证列表区域 `flex: 1` 铺满，短内容不露灰。
- `ListHeaderComponent` **第一块**为工具行；其下仍为库存卡 → 流水汇总 → 日列表（既有结构保留，仅时间控件形态与位置变）。
...
- **Prior art**：`summary-tab.test.tsx`、`date-format.test.ts`、`build-staff-workbook.test.ts`、`manage-tab.test.tsx` 导出段、`inventory.test.ts`。
...
- Grill（2026-08-04）：起止日+快捷下拉；默认近10天；工具行置顶；四 sheet 口径与列；配置 SQLite 即时保存；文件名带区间；排除作废；白底仅汇总页。

## Comments

- 2026-08-04 — draft from grilling; awaiting adversarial review before publish.
