...
- [ ] 默认列表不含已删除会员/商品；打开「包含删除」后出现已删除行且可恢复 —— 行为变化 + 恢复能力
- [ ] 搜索 + 开关关：只命中有效；开关开：可命中已删除 —— search 尊重 includeVoided
- [ ] 会员列表/搜索始终不含管理员 `-1`
- [ ] 补货 / 配置段无「包含删除」控件
- [ ] 既有 create/edit/void/restore 路径不回归（开关关时 void 后行消失，开时可见「已删除」+恢复）
